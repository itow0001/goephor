...
class Run(object):
    ''' This class represents a remote machine and uses SSH to perform all needed actions
    '''
    def __init__(self,server,rsa_private,user,password,strict=True,verbose=True, show_cmd=True):
        ...
        if not self.has_access():
            ...
            self.set_rsa()
            if not self.has_access():
                print "\n[Error] unable to add rsa key to host %s" % (self.server)
                sys.exit(1)
    ...
    def is_alive(self):
        ...
        @return: boolean
        """
        session = shell('/usr/bin/env ping -c 1 %s' % (self.server),strict=False,verbose=False,show_cmd=False)
    ...
    def is_writable(self):
        ...
        cmd = 'touch %s' % (file)
        session = ssh(self.server,cmd,rsa_private=self.rsa_private,
                      user=self.user,password=self.password,strict=False,verbose=False,add_rsa=False,show_cmd=False)
        if session.get('code') == 0:
            return True
        return False
    ...
    def set_rsa(self):
        ...
        cmd = 'hostname'
        session = ssh(self.server,cmd,rsa_private=self.rsa_private,
                      user=self.user,password=self.password,strict=False,verbose=False,add_rsa=True,show_cmd=False)
    ...
    def cmd(self,cmd):
        ...
        """
        session = ssh(self.server,cmd,rsa_private=self.rsa_private,
                     add_rsa=False,user=self.user,password=self.user,strict=self.strict,verbose=self.verbose)
        return session
...
if __name__ == '__main__':
    run = Run('10.25.70.62','/ifs/home/iitow/.ssh/id_rsa','root','a')
    print run.onefs_version()
    #run.cmd('ls -al').get('stdout')
    #print run.has_file('/root', '.login')
    #print run.find('/', 'lib')
    #print run.get_MD5('/root/.login')
"""
