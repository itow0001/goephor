...
from os import fork, waitpid, execv, read, write,wait
...
def set_rsa(host,rsa_pub,user,password):
    ...
    command = ['/usr/bin/ssh',user+'@'+host,'-o','NumberOfPasswordPrompts=1',cmd]
    pid, fd = pty.fork()
    if pid == 0:
        os.execvp("/usr/bin/ssh", ["ssh", user+'@'+host,
...
def ssh(server,cmd,rsa_private='/root/.ssh/id_rsa.default',add_rsa=False,user='root',password='a',strict=True,verbose=True,show_cmd=True,shell_set=True):
    ...
    try:
        output = shell("ssh -o LogLevel=quiet -o NumberOfPasswordPrompts=0 -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i %s %s@%s \"%s\"" 
                       % (rsa_private,user,server,cmd),strict=strict,verbose=verbose,show_cmd=show_cmd,shell=shell_set)
    except IOError as e:
        print "Error in ssh: %s" % (e)
    return output

def rsync(server,src,dest,option='pull',remote=True,excludes=[],rsa_private='/root/.ssh/id_rsa.default',user='root',verbose=False):
    ...
    if remote==True:
        if option=='pull':
            try:
                session = shell("rsync -e \"ssh -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null -i %s\""
                      " -Pavz %s --delete %s@%s:%s %s" % (rsa_private,excludes_str,user,server,src,dest),strict=True,verbose=verbose,shell=True)
            except IOError as e:
                print "Error in rsync: %s" % (e)
        elif option=='push':
            try:
                session = shell("rsync -e \"ssh -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null -i %s\""
                      " -Pavz %s --delete %s %s@%s:%s" % (rsa_private,excludes_str,src,user,server,dest),strict=True,verbose=verbose,shell=True)
            except IOError as e:
                print "Error in rsync: %s" % (e)
    elif remote==False:
        try:
            session = shell("rsync -Pavz %s --delete %s %s" % (excludes_str,src,dest),strict=True,verbose=verbose,shell=True)
        except IOError as e:
            print "Error in rsync: %s" % (e)
    else:
...
def shell(cmd,verbose=True, strict=False,shell=False,buffer_size=1048576,show_cmd=True):
    ...
    if shell==False:
        cmd = shlex.split(cmd)
    ### 2.6 does not support define multiple io,opens must be nested
