...
def md5_file(path):
    ...
    @example: {"core.actionable":{"md5_file":{"args":["/path/example.txt","/path/example.txt.md5"]}}}
    """
...
def symlink(src,dest):
    ...
    @param dest: new file path
    @return: shell session dictionary
    @example: {"core.actionable":{"symlink":{"args":["/path/example","/path2/example"]}}}
    """
...
def has_file(path,negate=False):
    ...
    if negate:
        ...
        session = shell("[ -f %s ]" % (path) ,strict=True,shell=True)
    return session
def scm_checkout(src_path,repo_url,branch,clean=True,user="build",passwd="a"):
    ...
    if clean==True:
        ...
        mkdir(src_path)
    if "git" in repo_url:
        session = shell("cd %s; git clone %s -b %s" % (src_path,repo_url,branch),strict=True,shell=True)
    elif "svn" in repo_url:
        session = shell("cd %s; svn co --username %s --password %s --non-interactive %s %s" % (src_path,user,passwd,repo_url,branch),strict=True,shell=True)
    else:
        print "[Error] unable to perform scm checkout"
    return session
def rsyncable(server,src,dest,option='pull',remote=True,excludes=[],rsa_private='/root/.ssh/id_rsa.default',user='root',verbose=False):
    """ perform a rsync
    ...
    @return: shell session dictionary
    """
    session = rsync(server,src,dest,option=option,remote=remote,excludes=excludes,rsa_private=rsa_private,user=user,verbose=verbose)
    return session

def jexec(jail_name,env,cmd):
...
def mount(opts,src,dest):
    ...
    if not src and dest in session.get('stdout'):
        ...
        session = shell("/sbin/mount %s %s %s" % (opts,src,dest),strict=True,shell=True)
    return session
...
def umount(opts,dest):
    ...
    @return: session   
    """
    session = shell("/sbin/mount %s %s" % (opts,dest),shell=True)
    if  dest in session.get('stdout'):
        print "[umount] unmounting %s %s" % (dest)
        session = shell("/sbin/umount %s %s" % (opts,dest),strict=True,shell=True)
    return session

def shell_cmd(cmd,verbose=True, strict=True,shell=True,buffer_size=1048576,show_cmd=True):
    ...
    """
    session = shell(cmd,verbose=verbose, strict=strict,shell=shell,buffer_size=1048576,show_cmd=show_cmd)
    return session
