...
class Repo_actions(object):
        ...
        if os.path.exists(ssh_config):
            with open(ssh_config,'r') as file:
                ...
                if git_host not in config:
                    ...
                    shell("sudo printf 'Host %s\\n\\tStrictHostKeyChecking no\\n' >> %s" % (git_host,ssh_config),shell=True)
        else:
    ...
    def clone(self,remote_ssh,branch='master'):
        ...
        try:
            ...
            repo = Repo.clone_from(remote_ssh,self.repo_path,branch=branch)
            self.repo = repo
            return True
        except Exception as e:
...
class Commit_actions(object):
    ...
    def diff_tree(self,sha1_str):
        if not self.repo:
            ...
            return False
        try:
            output = self.repo.git.diff_tree('--no-commit-id','--name-only','-r',sha1_str)
            return output
        except Exception as e:
            print e
        return None
...
class Remote_actions(object):
    ...
    def add(self,remote,name='upstream'):
        ...
        if not self.repo:
            ...
            return False
        try:
            self.repo.git.remote('add',name,remote)
            return True
        except Exception as e:
            print e
        return False
...
if __name__ == '__main__':
    r1 = Repo_actions('../../../deleteme')
    remote = "git@github.west.isilon.com:iitow/scm-tools.git"
    remote_url = 'https://github.west.isilon.com/iitow/scm-tools.git'
    branch_name = 'test1'
    #r1.clone(remote)
    r1.init()
    print r1.untracked_files()
    r1.branch_list()
    r1.branch(branch_name)
    r1.branch_list()
    r1.checkout(branch_name)
    r1.branch_is()
"""
